...
We then followed the evaluation procedure described above for each target gene in each of the three gene sets.

Figure {@fig:expression_gene_sets}B summarizes performance for all genes in each gene set.
Across all genes, the Vogelstein et al. gene set markedly improves the aggregate performance compared to the randomly chosen genes, and compared to choosing genes based on their total mutation count.
