...
That is, if a classifier for a given gene outperforms the permuted baseline for one data type, it is likely that it also outperforms the permuted baseline for other data types (corresponding to continuous vertical lines of boxes in the heatmap), supporting the observation that choosing the "correct" data modality is unimportant for driver genes with strong functional signatures.
