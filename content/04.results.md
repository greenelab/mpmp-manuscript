...
![
**A-F.** Classifier performance, relative to baseline with permuted labels, for mutation prediction models trained on various combinations of data types. For each dataset, we used the top 5,000 principal components as predictive features. "Multi-omics" models were formed by concatenating PC features derived from the relevant data types, then fitting the elastic net logistic regression model using the same process as for single-omics models.
**G.** Comparing the best-performing model trained on a single data type against the best "multi-omics" model for each target gene. None of the differences between single-omics and multi-omics models were statistically significant using paired-sample _t_-tests across cross-validation folds.
