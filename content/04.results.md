...
](images/figure_2.png){#fig:expression_gene_sets width="90%"}<!--Two questions: what is dark red vs brown red, and I would check that those colors can be distinguished for color blind readers (if they're meant to be different)-->
...
Confidence intervals between the best- and worst-performing data types overlap at most PC counts (with the exception of gene expression at 5,000 PC feature), suggesting that similarly to mutation prediction, the three data types tend to have comparable effectiveness for pan-cancer survival prediction.<!--Is there a word missing in the parenthetical? It reads a bit confusingly-->
...
Only non-zero overlap counts are shown.<!--Does this mean that the ones that look like 0 are > 0? Also, I think you used only a subset of these for your experiments, could you distinguish them somehow?-->
