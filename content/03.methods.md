...
This was mainly due to sample size limitations -- running a single comparison using all data types would force us to use only samples that are profiled for each data type, which would discard a large number of samples that are only profiled for one or a few data types.
