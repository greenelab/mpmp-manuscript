...
As an orthogonal validation of our mutation prediction results, we constructed predictors of patient survival using the clinical data available from the GDC, in the `TCGA-CDR-SupplementalTableS1.xlsx` file.
...
We measured survival prediction performance using the censored concordance index, described in [@pubmed:8668867], which quantifies agreement between the order of survival time predictions and true outcomes for a held-out dataset: higher concordance index values indicate more accurate survival prediction performance.
