...
where $i \in \{1, \dots, n\}$ denotes a given sample, $X_i \in \mathbb{R}^{p}$ denote features (omics measurements) and $y_i \in \{0, 1\}$ denote labels (mutation presence/absence), and $\ell(\cdot)$ denotes the negative log-likelihood of the observed data given a particular choice of feature weights, i.e.  
